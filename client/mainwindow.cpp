...
void MainWindow::paintEvent( QPaintEvent *event )
{
    ...
    painter.drawImage(0,this->menuBar()->geometry().height(),
                      pictures->get("field"));
    painter.drawImage(myField->getX(),this->menuBar()
                      ->geometry().height()+myField->getY(),
                      myField->getImage());
    painter.drawImage(enemyField->getX(),this->menuBar()
                      ->geometry().height()+enemyField->getY(),
                      enemyField->getImage());
}
    ...
    if (rx.indexIn(data)!=-1)
        {
            ...
            {
                if (rx.cap(2)=="half")
                    enemyField->setCell(rx.cap(3).toInt(),
                                        rx.cap(4).toInt(),
                                        CL_HALF);
            }
            myField->redraw();
