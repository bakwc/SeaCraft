...
void MainServer::parseData(const QString& cmd,int clientId)
{
    ...
    if (authorize(cmd,i)) return;
    if (setField(cmd,i)) return;
    if (makeStep(cmd,i)) return;
...
{               // Checking received data for authorization and making authorization
    ...
    if (rx.indexIn(cmd)!=-1)
        ...
        {
            qDebug() << "Version: " << rx.cap(1);   // TODO: check for version
            qDebug() << "Login: " << rx.cap(2);
            qDebug() << "Password: " << rx.cap(3);

            client->status=ST_AUTHORIZED;
            client->send("mbserver:1:");  // TODO: take version from defines
            return true;
        }
    return false;
}
...
{
    if (!Field::isFieldCorrect(ships)) return false;
...
void MainServer::connectTwoClients(Clients::iterator client1,
